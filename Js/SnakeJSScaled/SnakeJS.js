...
function setup() {
  this.wid = floor((windowWidth) /20);
  this.hei = floor((windowHeight - 80) /20);
  console.log(wid);
  createCanvas(wid*20, hei*20 + 40);
  stroke(0);
  background(255);
  console.log("test");
  jake = new Snake();
  fruit = new Cherry();
  for(let sizeOfArr = 0; sizeOfArr < wid; sizeOfArr++){
    map[sizeOfArr] = [];
  }
  for(let j = 0; j < wid; j++){
    for(let k = 0; k < hei; k++){
...
function draw() {
  if(dead){
    textSize(40);
    text("YOU DIED", wid*10-70, hei*10);
    textSize(20);
  }
